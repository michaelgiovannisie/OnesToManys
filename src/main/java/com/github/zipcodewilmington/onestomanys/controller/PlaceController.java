...
public class PlaceController {
    ...
    public List<Place> getPlacesByDestinationId(@PathVariable Integer id) {
        Destination destination =
                destinationRepository.findById(id).orElse(null);

        if (destination == null) {
            return List.of();
        }

        return place.findByDestinationIdOrderByArrivalTimeAsc(id);
    }
}
